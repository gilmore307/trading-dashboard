...
- Historical Task Progress current content is being completed before opening broader dashboard pages: Tasks now focuses on a filtered historical task list. The default view shows only current `Now` work, with filters for layer, status, and task/work type; model-specific month/stage/progress/coverage cards live under Models.
