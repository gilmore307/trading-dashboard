...
- First website slice is implemented: Vite + React + TypeScript renders a public read-only Tasks / Historical Modeling page from `historical_task_progress_summary_v1`, with the left navigation as the only page-switching entry point plus read-only refresh and in-view diagnostics expansion.
