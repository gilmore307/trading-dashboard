...
- Timewheel consumes `temporal_explorer_summary` from storage. The page shows substrate population as a status card, provides a selectable date/tick Timeline Status panel, treats the chart x-axis as the Timewheel with local symbol/frame/center-time controls, shows volume and accepted-event-density subcharts below the primary chart, and displays only the selected time unit's Layer 10 accepted event markers. Market-state summary belongs on Status rather than taking a separate Timewheel lane.
