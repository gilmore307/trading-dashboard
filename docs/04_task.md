...
- Replay Attribution consumes replay-review attribution from `model_group_replay_review_summary`. The page focuses on replay-process errors, miss-attribution layers, cause families, failure types, first-gap components, and whether replay error rows publish event references. It does not claim event causality until review rows expose event candidate refs.
