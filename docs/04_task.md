...
- Dashboard continues consuming storage-hosted summaries rather than raw internals. Current Status now includes a Multitask Threads card for scheduler/provider parallelism: selected workers, configured max workers, provider request batch limit, idle/backstop tick interval, drain mode/limits, live read-model refresh posture, load target, and memory budget.
