...
- Events consumes `temporal_explorer_summary` as its event attention-pool surface. The page shows substrate population as a status card, uses a TradingView-style K-line chart with frame-aligned local symbol/frame/center-time controls, shows volume and accepted-event-density subcharts, displays certified event-family markers for the selected time unit, and summarizes certified event-family to market-session/chart-state pairings. Events no longer consumes replay residual-event governance.
