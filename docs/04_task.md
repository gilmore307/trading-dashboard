...
- Timewheel consumes `temporal_explorer_summary` from storage. The page shows a chart viewport, synchronized vertical time ticks, left-side market/session lanes, right-side event lanes, visible event markers, chart-cache posture, and explicit gaps for unpopulated Temporal Explorer substrate tables.
