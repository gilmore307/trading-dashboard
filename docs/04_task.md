...
- Historical Task Progress current content is being completed before opening broader dashboard pages: Tasks now focuses on a filtered, month-grouped historical child-task list. The default view shows only current `Now` work, with filters for layer, status, and task/work type; each row can expand to show task detail/progress. Model-specific month/stage/progress/coverage cards live under Models.
