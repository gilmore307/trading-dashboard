...
- Events consumes `temporal_explorer_summary` as its event attention-pool surface. The page uses a TradingView-style K-line chart over the current model-group replay window, with SPY/QQQ/IWM/DIA symbol selection and 1D/1W frame selection, shows volume and accepted-event-density subcharts, displays certified event-family markers for the selected time unit, and summarizes certified event-family to market-session/chart-state pairings. Events no longer exposes substrate-status cards and no longer consumes replay residual-event governance.
