...
- Dashboard continues consuming storage-hosted summaries rather than raw internals. Current Status now includes a Runtime Throughput card: 3 month-ingest + 1 model-worker topology, six-month fold cadence, completion rate, peak completion burst, observation window, and idle/blocked decision count. Provider-thread settings remain subordinate implementation detail rather than the primary card content.
