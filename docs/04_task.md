...
- Events consumes `temporal_explorer_summary` as its primary event timeline and appends replay residual-event governance from `model_group_replay_review_summary` when published. The page shows substrate population as a status card, uses a TradingView-style K-line chart with frame-aligned local symbol/frame/center-time controls, shows volume and accepted-event-density subcharts, displays only the selected time unit's M06 accepted event markers, and then shows replay event-focus/governance summaries when available. Market-state summary belongs on Status rather than taking a separate temporal lane.
