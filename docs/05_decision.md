...
The Replay Decisions layer metric set separates continuous layer coverage, full candidate-trace rows, and outcome-labeled reached-layer rows. Triggered / Coverage counts show replay timestamp coverage for continuous context layers such as M01 and M03 when published. M01-M03 focused ledgers use the full point-in-time candidate trace and must not be filtered by downstream selected trades; until future labels are joined to that same full-trace row set, they remain unscored diagnostics. M04-M05 rows use only decisions that reached those layers and can carry post-replay labels for correctness/regret judgment. Correct % and Incorrect % own post-replay correctness labels. Acceptable % is treated as an acceptability rate, not accuracy. Harm % remains the primary harmful-error rate. Missed %, Mean Impact, and worst-regret context are displayed alongside Mean Regret so tails and missed-good decisions are visible.
...
- M01 reviews point-in-time background/context state over the full candidate trace. It must not use later selected trades to filter the M01 denominator. Future labels may score M01 only after they are joined to the same full-trace row set; the future label is review-only and must not become a decision-time input.
