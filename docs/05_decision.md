...
The Data page displays `event_category_type` as the user-facing `event_type` column and places it first for the M03 event-governance events table. `information_role_type` is displayed as `information_role`, `scope_type` as `event_scope`, and `source_name` as `event_source`. Raw column names remain available in the column header detail text for traceability.
...
The Replay Decisions layer metric set separates continuous layer coverage, targetless M01 background-state rows, M02 candidate-trace rows, M03 interpreted event/effect rows, and outcome-labeled reached-layer rows. M01 focused ledgers use one targetless background-context row per replay timestamp; they must not inherit target symbols from later candidate expansion. M02 focused ledgers use the full point-in-time candidate trace and must not be filtered by downstream selected trades. M02 owner-facing tables emphasize rank, selected share, and candidate scores. M03 focused ledgers use the point-in-time interpreted event/effect ledger, one event effect per row; they must not be derived from M02 target-candidate expansion or downstream selected trades. M03 owner-facing tables emphasize event-impact disposition counts such as no impact, context only, risk shape, directional effect, and unresolved. Until future labels are joined to the same layer row set, M01-M03 remain unscored diagnostics and should show diagnostic coverage/evidence gaps instead of outcome-score columns. M04-M05 rows use only decisions that reached those layers and can carry post-replay labels for correctness/regret judgment. Correct % and Incorrect % are complements, so the primary table shows Correct % only alongside Harm %, Missed %, Mean Regret, and Mean Impact. Acceptable % remains available as a future narrower acceptance concept but is not a required primary column when it equals correctness.
...
- M01 reviews point-in-time background/context state at replay-time granularity. It must not use later selected trades or target-candidate expansion to define the M01 denominator. Future labels may score M01 only after they are joined to the same targetless background-state row set; the future label is review-only and must not become a decision-time input.
- M02 reviews same-timestamp target selection, rank, direction, and tradability.
- M03 reviews event pressure and risk-state diagnostics available at decision time over the interpreted event/effect ledger. The M03 denominator is the event/effect ledger: one point-in-time interpreted event effect per row. It must not use M02 target-candidate expansion or later selected trades to define the denominator. Future labels may score M03 only after they are joined to the same event/effect row set; the future label is review-only and must not become a decision-time input. No-impact events remain visible as neutral rows instead of disappearing from the denominator.
