...
The Replay Decisions layer metric set separates continuous layer coverage, targetless M01 background-state rows, M02 candidate-trace rows, M03 event-pool rows, and outcome-labeled reached-layer rows. M01 focused ledgers use one targetless background-context row per replay timestamp; they must not inherit target symbols from later candidate expansion. M02 focused ledgers use the full point-in-time candidate trace and must not be filtered by downstream selected trades. M02 owner-facing tables emphasize rank, selected share, and candidate scores. M03 focused ledgers use the point-in-time event observation pool, one event per row; they must not be derived from M02 target-candidate expansion or downstream selected trades. Until future labels are joined to the same layer row set, M01-M03 remain unscored diagnostics and should show diagnostic coverage/evidence gaps instead of outcome-score columns. M04-M05 rows use only decisions that reached those layers and can carry post-replay labels for correctness/regret judgment. Correct % and Incorrect % are complements, so the primary table shows Correct % only alongside Harm %, Missed %, Mean Regret, and Mean Impact. Acceptable % remains available as a future narrower acceptance concept but is not a required primary column when it equals correctness.
...
- M03 reviews event pressure and risk-state diagnostics available at decision time over the event observation pool. The M03 denominator is the event pool: one point-in-time event observation per row. It must not use M02 target-candidate expansion or later selected trades to define the denominator. Future labels may score M03 only after they are joined to the same event-pool row set; the future label is review-only and must not become a decision-time input.
