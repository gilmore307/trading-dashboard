...
The Replay Decisions layer metric set separates continuous layer coverage from outcome-labeled reviewed rows. Triggered / Coverage counts show the replay timestamp coverage for continuous context layers such as M01 and M03 when published. Reviewed rows show the selected-path decisions that carry post-replay labels and can support correctness/regret judgment. Correct % and Incorrect % own post-replay correctness labels. Accept % is treated as an acceptability rate, not accuracy. Harm % remains the primary harmful-error rate. Missed %, Mean Impact, and worst-regret context are displayed alongside Mean Regret so tails and missed-good decisions are visible.
...
- M01 reviews point-in-time background/context state using post-replay selected-path outcome labels to judge whether accepting that context state was correct. The future label is review-only and must not become a decision-time input.
- M02 reviews same-timestamp target selection, rank, direction, and tradability.
- M03 reviews event pressure and risk-state diagnostics available at decision time using post-replay selected-path outcome labels to judge whether accepting, withholding, or downweighting that event state was correct. The future label is review-only and must not become a decision-time input.
