...
The Replay Decisions layer metric set separates continuous layer coverage, targetless M01 background-state rows, M02 candidate-trace rows, M03 event-pool rows, and outcome-labeled reached-layer rows. M01 focused ledgers use one targetless background-context row per replay timestamp; they must not inherit target symbols from later candidate expansion. M02 focused ledgers use the full point-in-time candidate trace and must not be filtered by downstream selected trades. M03 focused ledgers use the point-in-time event observation pool, one event per row; they must not be derived from M02 target-candidate expansion or downstream selected trades. Until future labels are joined to the same layer row set, M01-M03 remain unscored diagnostics. M04-M05 rows use only decisions that reached those layers and can carry post-replay labels for correctness/regret judgment. Correct % and Incorrect % own post-replay correctness labels. Acceptable % is treated as an acceptability rate, not accuracy. Harm % remains the primary harmful-error rate. Missed %, Mean Impact, and worst-regret context are displayed alongside Mean Regret so tails and missed-good decisions are visible.
