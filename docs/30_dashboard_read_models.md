...
The dashboard renders Status, Tasks, Data, Model Groups, Event Families, Replay Performance, Replay Decisions, Replay Operations, Replay Attribution, Diagnostics, and Realtime Signals without querying raw internals for primary page content.
...
Purpose: support the Event Families page as the read-only event-ontology and event-ledger summary.

Current implementation: `trading-storage` may build this summary from accepted event-attention-pool inputs, market-session context, and ETF chart bars for the current model-group replay window. The dashboard subscribes to it for the Historical Models / Event Families page and uses its `events` and `event_families` arrays to render a selectable event-family hierarchy plus the event rows covered by the selected node.
...
- event-family hierarchy fields when published, from coarse source/domain levels down to mechanism, submechanism, and specific event dossiers;
- event risk or impact scores when published by upstream M03 event-state evidence;
...
| M05 | Option Expression | `expression_probability_surface`, derived `option_expression_plan` |
...
- Replay Performance also consumes `review_runs[].standard_review_diagnostics` as supporting economic context: candidate-entry funnel conversion, option-expression unexecutable counts, selected-candidate rank, M05 filled-good/bad counts, and mechanism-contract breach counts. These diagnostics are comparison context for why performance rose or fell; they do not replace headline NAV, return, drawdown, or filled-trade metrics;
- Replay Decisions consumes `review_runs[].replay_decisions_m01_m05` as its primary contract; the page renders five separate M01-M05 layer chapters, each with its own layer-quality comparison table, charts, and focused paginated effective layer-decision ledger. Current runs read dedicated `layer_review_rows_ref` evidence. M01 rows are targetless replay-time background-context state rows and must not inherit target symbols from later candidate expansion. M02 rows come from the full point-in-time model-candidate trace and must not be filtered by later selected trades. M03 rows come from the point-in-time interpreted event/effect ledger, one event effect per row; M03 must not be synthesized from M02 target-candidate expansion or downstream selected trades. M03 summary rows publish event-impact disposition counts, including no impact, context only, risk shape, directional effect, and unresolved. M04-M05 rows come from decisions that reached those layers. Rows are scored by layer-specific method and evidence: M01-M03 remain unscored until future labels are joined to their own layer row set, M04 uses directional underlying post-replay labels, and M05 uses option-expression/contract-path post-replay labels. Future labels are review labels only, not decision-time inputs. Replay decision-row synthesis is only a legacy fallback; `decision_review` and `parameter_review` remain supporting attribution/parameter context, not the primary correctness surface;
- Replay Operations consumes `review_runs[].replay_operations_c01_c07` as its primary contract and uses the same model-group selector pattern as Replay Performance and Replay Decisions, then renders C01-C07 component tabs. Summary mode compares the active component across model groups with component-specific evidence fields, metric families, required evidence status, and component verdict. Focus mode shows the selected model group's replay K-line as the main chart, overlays operation rows as marks, lets mark selection switch the active component and filter the concrete operation ledger, and then shows component cards, supporting operation charts, summary rows, and concrete operation details. Current data comes from completed post-replay review layer-attribution artifacts: `operation_component_action_rows.csv` is the primary focused ledger, while `operation_component_flow.csv`, `operation_component_review_packet.csv`, and `operation_component_metrics.csv` supply summaries and auxiliary metric cards. Each operation row carries `operation_action`, `operation_status`, input/output summaries, block reason, `analysis_method`, `evidence_role`, `label_role`, trigger state, point-in-time feasible action-set reference/count/status, component objective, chosen action, best available post-replay label action, ex-post rank when known, correctness class, label basis, and optional realized-return/regret labels. Future facts are review labels only and may rank only actions visible and feasible at decision time. C03 lifecycle must publish portfolio lifecycle/replacement evidence or a lifecycle evidence gap; it is not hidden as not applicable. C04 Expression Review covers options-first equity expression, option rolling/replacement, crypto leverage expression, selected path materialization, and direct-underlying fallback eligibility. `decision_review.first_gap_component_counts` and `decision_review.sample_rows` remain attribution evidence and do not backfill the Operations concrete ledger;
- Replay Attribution consumes `review_runs[].decision_review` as model/replay failure attribution evidence and uses the same model-group selector pattern as Replay Performance, Replay Decisions, and Replay Operations. It renders cause-family, failure-type, miss-attribution-layer, and first-gap-component statistics plus all published concrete failure rows. It is not an event-ontology page; event-related causes are only one possible cause family;
- Model Groups consumes model lifecycle, promotion, readiness, calibration, ranking, feature-space, data-integrity, temporal-stability, and shadow/active posture summaries only. It must not consume replay returns, operation traces, component action rows, duplicate operation-trace diagnostics, or attribution verdicts as primary content. Replay return evidence belongs to Replay Performance, layer correctness belongs to Replay Decisions, operation action evidence belongs to Replay Operations, and root-cause verdicts belong to Replay Attribution;
- `chart_payload.cross_model_group_diagnostics` publishes duplicate trace groups over normalized replay operation action rows so the dashboard can flag non-independent replay evidence across model groups without treating it as trading performance;
