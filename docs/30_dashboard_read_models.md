...
- Replay Operations consumes `review_runs[].replay_operations_c01_c07` as its primary contract and uses the same model-group selector pattern as Replay Performance and Replay Decisions, then renders C01-C07 component tabs. Summary mode compares the active component across model groups with component-specific evidence fields, metric families, required evidence status, and component verdict. Focus mode shows the selected model group's component metric cards, component artifact charts, and metric ledger. Current data comes from completed post-replay review layer-attribution artifacts such as `operation_component_flow.csv`, `operation_component_review_packet.csv`, and `operation_component_metrics.csv`; each metric row carries `metric_family`, `analysis_method`, `evidence_role`, `label_role`, and `required_evidence_status`. C03 lifecycle must publish portfolio lifecycle/replacement evidence or a lifecycle evidence gap; it is not hidden as not applicable. `decision_review.first_gap_component_counts` and `decision_review.sample_rows` are only legacy fallback evidence;
