...
- Replay Operations consumes `review_runs[].replay_operations_c01_c07` as its primary contract and uses the same model-group selector pattern as Replay Performance and Replay Decisions, then renders C01-C07 component tabs. Summary mode compares the active component across model groups with input rows, output rows, blocked rows, eligible rows, first-limiting rows, metric rows, data-gap metric counts, hit rate, mean return, and component verdict. Focus mode shows the selected model group's component metric cards, component artifact charts, and metric ledger. Current data comes from completed post-replay review layer-attribution artifacts such as `operation_component_flow.csv`, `operation_component_review_packet.csv`, and `operation_component_metrics.csv`; `decision_review.first_gap_component_counts` and `decision_review.sample_rows` are only legacy fallback evidence;
