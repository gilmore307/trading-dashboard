...
- task timeline listing past, current, and future child-task rows at `month/fold + layer + phase` granularity, with canonical phase labels such as data acquisition, feature generation, model generation, model evaluation, Promotion Review, and maintenance; source/feature stages that originate from a six-month fold are projected into their month rows, while model stages remain on the training fold; model-group replay rows keep the training fold as a `YYYY-foldN` period such as `2016-fold1` and expose exact training/replay windows separately in detail; month-scoped rows are capped at the latest completed calendar month in `America/New_York`, so the current incomplete month is not exposed as a Ready task; the dashboard groups this timeline by month/fold, filters it by month/layer/status/task type, orders filter choices by chronological/workflow sequence rather than label alphabetization, defaults to current `Now` work, shows the owner/evaluation/execution worker directly on every collapsed task preview, supports worker filtering, and can expand each row using sanitized detail fields including worker identity plus generated, started, ended, and status-updated timestamps when available; task progress is shown only from real evidence such as row counts, replay/month counts, elapsed/expected time, stage coverage, or worker-written active progress files under manager runtime;
...
Current implementation: `trading-storage` builds this summary from accepted Temporal Explorer substrate tables, chart cache, execution runtime status, and replay artifact root. The dashboard treats the chart x-axis as the Timewheel, displays event markers on that same axis, shows lower subcharts such as volume and event density, exposes right-side event/status lanes, and keeps explicit empty/not-populated states. `chart_ohlcv_cache` is shown as visualization cache only, not training truth.
...
- right lanes for scheduled events, event results, news index, model event markers, and replay state;
...
- calendar window and event counts;
- upcoming and recent event rows;
- source-artifact evidence count;
- family rows for macro scheduled releases, earnings scheduled shells, exchange holidays and early closes, option expiry windows, and index rebalance windows;
- refresh status for the TE schedule/worker and latest TE source receipt/file timestamps.

Hidden by default:

- raw SQL row dumps;
- raw Trading Economics CSV files and receipts;
- provider credentials or source secret paths;
- dashboard-originated event refresh controls.
