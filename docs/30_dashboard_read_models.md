...
- Replay Operations uses the same model-group selector pattern as Replay Performance and Replay Decisions, then renders C01-C07 component tabs. Summary mode compares the active component across model groups with gap rows, sample rows, gap rate, mean regret, mean impact, cause-family count, failure-type count, and top mechanism. Focus mode shows the active component's time-axis trend charts, summary row, and sample ledger. Current data comes from `decision_review.first_gap_component_counts` and `decision_review.sample_rows`; missing component evidence remains an empty state until the storage/review producer publishes a full C01-C07 component ledger;
