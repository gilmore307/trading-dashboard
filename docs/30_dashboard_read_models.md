...
- Replay Decisions consumes `review_runs[].replay_decisions_m01_m05` as its primary contract; the page renders five separate M01-M05 layer chapters, each with its own layer-quality comparison table, charts, and focused paginated effective layer-decision ledger. Current runs read dedicated `layer_review_rows_ref` evidence with one row per replay decision per included layer. Continuous context layers such as M01 and M03 also consume coverage counts from replay trace timestamps, so trigger coverage remains separate from outcome-labeled selected-path review rows. Rows are scored by layer-specific method and evidence: M01 uses post-replay selected-path labels to review accepted background context, M02 uses point-in-time target/candidate diagnostics, M03 uses post-replay selected-path labels to review accepted event state, M04 uses directional underlying post-replay labels, and M05 uses option-expression/contract-path post-replay labels. Future labels are review labels only, not decision-time inputs. Replay decision-row synthesis is only a legacy fallback; `decision_review` and `parameter_review` remain supporting attribution/parameter context, not the primary correctness surface;
