...
- Model Groups consumes only the model-integrity slice of `standard_review_diagnostics` and `cross_model_group_diagnostics`: review completeness, M05 expression-state coverage, mechanism-contract breach counts, and normalized replay-trace duplication risk. Trading impact from those diagnostics belongs to Replay Performance, layer correctness belongs to Replay Decisions, and operation action evidence belongs to Replay Operations;
