...
- Replay Decisions consumes `review_runs[].replay_decisions_m01_m05` as its primary contract; the page renders five separate M01-M05 layer chapters, each with its own layer-quality comparison table, charts, and focused effective layer-decision ledger. Current runs read dedicated `layer_review_rows_ref` evidence with one row per replay decision per included layer. Rows are scored by layer-specific method and evidence: M01 uses point-in-time background-context usability thresholds, M02 uses point-in-time target/candidate diagnostics, M03 uses point-in-time event-pressure/downweight thresholds, M04 uses directional underlying post-replay labels, and M05 uses option-expression/contract-path post-replay labels. Replay decision-row synthesis is only a legacy fallback; `decision_review` and `parameter_review` remain supporting attribution/parameter context, not the primary correctness surface;
