...
- Replay Decisions consumes `review_runs[].replay_decisions_m01_m05` as its primary contract; the page renders five separate M01-M05 layer chapters, each with its own layer-quality comparison table, charts, and focused paginated effective layer-decision ledger. Current runs read dedicated `layer_review_rows_ref` evidence. M01 rows are targetless replay-time background-context state rows and must not inherit target symbols from later candidate expansion. M02-M03 rows come from the full point-in-time model-candidate trace and must not be filtered by later selected trades; M04-M05 rows come from decisions that reached those layers. Continuous context layers such as M01 and M03 also consume coverage counts from replay trace timestamps, so trigger coverage remains separate from full candidate-row counts. Rows are scored by layer-specific method and evidence: M01-M03 remain unscored until future labels are joined to their own layer row set, M04 uses directional underlying post-replay labels, and M05 uses option-expression/contract-path post-replay labels. Future labels are review labels only, not decision-time inputs. Replay decision-row synthesis is only a legacy fallback; `decision_review` and `parameter_review` remain supporting attribution/parameter context, not the primary correctness surface;
